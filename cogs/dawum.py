...
class Dawum(commands.Cog):
    ...
    def umfrage_ausgeben(self, parlacode, count):
        ...
        if parlacode.isdigit():
            umfragenid = [parlacode] if int(parlacode) > 100 else [int(k) for k, v in data['Surveys'].items()
                                                                   if v['Parliament_ID'] == str(parlacode.lower())]
        else:
    ...
    async def poll(self, ctx, parla="bt", count=1):
        ...
        if parla.lower() == "help":
            ...
            if type(result) is not discord.Embed:
                await ctx.send(f"Parlament/Umfragenid wurde nicht gefunden! Dein Code: {result}")
                return
